...
for i = 1:steps
    ...
    if bifurcation_detected == 1
        ...
        %save p1n5m2_bif load_solution load_tangent;
...
scatter3(vectorized_solutions(1,1:n*m),vectorized_solutions(2,1:n*m),vectorized_solutions(3,1:n*m),30, 'b','filled')
...
if solution_proof == 1
    subplot(2,2,2)
    scatter(1:steps,proof_data,10,color2(2:end,:),'filled')
    hold on
    scatter(1:steps,r_stars, 2, 'k')    
    set(gca,'FontSize',15)
    axis ([0 inf 0 inf])
    set(gca,'yscale','log')
...
hold on
scatter(momentum(1), energy(1), 30, 'b', 'filled')
set(gca,'FontSize',15)
xlabel('$$\mu$$', 'Interpreter', 'latex', 'FontSize', 25)
