...
load_tangent = zeros(4*n+2, 1);
%}



starting_step_size = 1e-2;
...
if existence_proof
    proof_data_point(1) = interval_f_U(fixed_initial_solution, m, poles, u_tilde, r_star);
end
...
elseif err == 4
    k = [0 0 1];    %blue - only used for testing
end
...
disp(['Start',', norm of inverse = ' , num2str(norm((DF_arc(solutions(:,1), m, poles,u_tilde, tangents(:,1)))^-1,inf)), ', omega = ', num2str(solutions(end,1))])
%Continuation
i = 1;
end_segment = 0;
while i <= steps
    ...
    %{
    if i == 1 && segment == 2 && tangents(end,i+1) > 0
        tangents(:,i+1) = -tangents(:,i+1);
    end
    %}
    ...
    if existence_proof
        ...
        proof_data_point(i+1) = interval_f_U(solutions(:,i), m, poles, u_tilde, r_star);
